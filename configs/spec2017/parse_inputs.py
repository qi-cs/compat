...
def get_input_dict_file(file_name):
    ...
    for line in text.split("\n"):
        ...
        if m:
            ...
            continue
        cmd = re.split(r"\s+", line.split(">")[0].strip())
        cmds[cur_bmk].append(cmd)

    return cmds, name_dict


def get_input_dict(t="all", size="ref", option="rate"):
    if t == "int" or t == "fp":
        return get_input_dict_file("./spec2017-{}rate-{}.txt".format(t, "ref"))
    elif option == "speed":
        assert t == "all"
        all_bmks, all_names = get_input_dict_file(
            "./spec2017-{}speed-{}.txt".format("int", "ref")
        )
        fp_bmks, fp_names = get_input_dict_file(
            "./spec2017-{}speed-{}.txt".format("fp", "ref")
        )
        ...
        assert t == "all"
        all_bmks, all_names = get_input_dict_file(
            "./spec2017-{}rate-{}.txt".format("int", "ref")
        )
        fp_bmks, fp_names = get_input_dict_file(
            "./spec2017-{}rate-{}.txt".format("fp", "ref")
        )
        for k in fp_bmks:  # merge into int bmks
            all_bmks[k] = fp_bmks[k]
            all_names[k] = fp_names[k]
